...
export class TmuxSessions implements SessionManager {
  private store = new Map<string, SessionEntry>();
  readonly filePath: string;

  constructor(port?: number) {
    this.filePath = port != null ? `/tmp/cc2wechat-tmux-${port}.json` : '/tmp/cc2wechat-tmux.json';
    this.loadFromDisk();
  }
